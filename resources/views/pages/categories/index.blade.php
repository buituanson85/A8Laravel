...
@section('title', 'Pages Categories List')
@section('content')
    <!--Hero Section-->
    <div class="hero-section hero-background">
        <h1 class="page-title">{{ $category_name->name }}</h1>
    </div>
    ...
    <div class="container">
        <nav class="biolife-nav">
            <ul>
                ...
                <li class="nav-item"><a href="{{ route('single-products.index') }}" class="permal-link">Shop</a></li>
                <li class="nav-item"><span class="current-page">All Category {{ $category_name->name }}</span></li>
            </ul>
        </nav>
    </div>
                ...
                <div id="main-content" class="main-content col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="block-item recently-products-cat md-margin-bottom-39">
                        <ul class="products-list biolife-carousel nav-center-02 nav-none-on-mobile" data-slick='{"rows":1,"arrows":true,"dots":false,"infinite":false,"speed":400,"slidesMargin":0,"slidesToShow":5, "responsive":[{"breakpoint":1200, "settings":{ "slidesToShow": 3}},{"breakpoint":992, "settings":{ "slidesToShow": 3, "slidesMargin":30}},{"breakpoint":768, "settings":{ "slidesToShow": 2, "slidesMargin":10}}]}' >
                            @foreach($products_category_slider as $product_category_slider)
                                <li class="product-item">
                                    <div class="contain-product layout-02">
                                        <div class="product-thumb">
                                            <a href="{{ route('single-products.show', $product_category_slider->id) }}" class="link-to-product">
                                                <img src="{{ asset('assets/uploads/products') }}/{{ $product_category_slider->image }}" alt="dd" width="270" height="270" class="product-thumnail">
                                            </a>
                                        </div>
                                        <div class="info">
                                            <b class="categories">{{ $product_category_slider->category->name }}</b>
                                            <h4 class="product-title"><a href="#" class="pr-name">{{ $product_category_slider->name }}</a></h4>
                                            <div class="price">
                                                <ins><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_category_slider->sale_price }}</span></ins>
                                                <del><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_category_slider->regular_price }}</span></del>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                    ...
                    <div class="product-category grid-style">
                                ...
                                @foreach($products_category as $product_category)
                                    <li class="product-item col-lg-2 col-md-3 col-sm-4 col-xs-6">
                                        <div class="contain-product layout-default">
                                            <div class="product-thumb">
                                                <a href="{{ route('single-products.show', $product_category->id) }}" class="link-to-product">
                                                    <img src="{{ asset('assets/uploads/products') }}/{{ $product_category->image }}" alt="dd" width="270" height="270" class="product-thumnail">
                                                </a>
                                            </div>
                                            <div class="info">
                                                <b class="categories">{{ $product_category->category->name }}</b>
                                                <h4 class="product-title"><a href="#" class="pr-name">{{ $product_category->name }}</a></h4>
                                                <div class="price">
                                                    <ins><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_category->sale_price }}</span></ins>
                                                    <del><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_category->regular_price }}</span></del>
                                                </div>
                        ...
                        <div class="biolife-panigations-block">
                            {{ $products_category -> links('pagination::bootstrap-4') }}
                        </div>
