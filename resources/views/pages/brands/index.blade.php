...
@section('title', 'Pages Brand List')
@section('content')
    <!--Hero Section-->
    <div class="hero-section hero-background">
        <h1 class="page-title">{{ $brand_name->name }}</h1>
    </div>
    ...
    <div class="container">
        <nav class="biolife-nav">
            <ul>
                ...
                <li class="nav-item"><span class="current-page">All Brand {{ $brand_name->name }}</span></li>
            </ul>
        </nav>
    </div>
                ...
                <div id="main-content" class="main-content col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="block-item recently-products-cat md-margin-bottom-39">
                        <ul class="products-list biolife-carousel nav-center-02 nav-none-on-mobile" data-slick='{"rows":1,"arrows":true,"dots":false,"infinite":false,"speed":400,"slidesMargin":0,"slidesToShow":5, "responsive":[{"breakpoint":1200, "settings":{ "slidesToShow": 3}},{"breakpoint":992, "settings":{ "slidesToShow": 3, "slidesMargin":30}},{"breakpoint":768, "settings":{ "slidesToShow": 2, "slidesMargin":10}}]}' >
                            @foreach($products_brand_slider as $product_brand_slider)
                                <li class="product-item">
                                    <div class="contain-product layout-02">
                                        <div class="product-thumb">
                                            <a href="{{ route('single-products.show', $product_brand_slider->id) }}" class="link-to-product">
                                                <img src="{{ asset('assets/uploads/products') }}/{{ $product_brand_slider->image }}" alt="dd" width="270" height="270" class="product-thumnail">
                                            </a>
                                        </div>
                                        <div class="info">
                                            <b class="categories">{{ $product_brand_slider->category->name }}</b>
                                            <h4 class="product-title"><a href="#" class="pr-name">{{ $product_brand_slider->name }}</a></h4>
                                            <div class="price">
                                                <ins><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_brand_slider->sale_price }}</span></ins>
                                                <del><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_brand_slider->regular_price }}</span></del>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                    ...
                    <div class="product-category grid-style">
                                ...
                                @foreach($products_brand as $product_brand)
                                    <li class="product-item col-lg-2 col-md-3 col-sm-4 col-xs-6">
                                        <div class="contain-product layout-default">
                                            <div class="product-thumb">
                                                <a href="{{ route('single-products.show', $product_brand->id) }}" class="link-to-product">
                                                    <img src="{{ asset('assets/uploads/products') }}/{{ $product_brand->image }}" alt="dd" width="270" height="270" class="product-thumnail">
                                                </a>
                                            </div>
                                            <div class="info">
                                                <b class="categories">{{ $product_brand->category->name }}</b>
                                                <h4 class="product-title"><a href="#" class="pr-name">{{ $product_brand->name }}</a></h4>
                                                <div class="price">
                                                    <ins><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_brand->sale_price }}</span></ins>
                                                    <del><span class="price-amount"><span class="currencySymbol">£</span>{{ $product_brand->regular_price }}</span></del>
                                                </div>
                        ...
                        <div class="biolife-panigations-block">
                            {{ $products_brand -> links('pagination::bootstrap-4') }}
                        </div>
